...
//    self.descLabel.frame = CGRectMake(10, CGRectGetHeight(self.frame) - 100, CGRectGetWidth(self.frame)-20, 100);
    ...
    [view mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.descLabel).offset(-10);
    }];
}
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    NSString * str = [self.descArray objectAtIndex:index];
    self.descLabel.text = [NSString stringWithFormat:@"%ld/%ld  %@",index+1,self.imageCount,str];
    if (view.beginLoadingImage) return;
    if ([self CX_highQualityImageURLForIndex:index]) {
        [view setImageWithURL:[self CX_highQualityImageURLForIndex:index] placeholderImage:[self CX_placeholderImageForIndex:index]];
        
    } else {
        view.imageview.image = [self CX_placeholderImageForIndex:index];
    }
...
//    _contentView.bounds = view.bounds;
//
//    self.center = CGPointMake(_contentView.bounds.size.width * 0.5, _contentView.bounds.size.height * 0.5);
//    self.bounds = CGRectMake(0, 0, _contentView.bounds.size.width, _contentView.bounds.size.height);
...
//    [self performSelector:@selector(onDeviceOrientationChangeWithObserver) withObject:nil afterDelay:HZPhotoBrowserShowImageAnimationDuration + 0.2];
    ...
    [_contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(view).offset(kTopHeight);
...
- (void)showFirstImage
{
    ...
    tempView.frame = rect;
    tempView.image = [self CX_placeholderImageForIndex:self.currentImageIndex];
...
- (UIImage *)CX_placeholderImageForIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(CX_photoBrowser:placeholderImageForIndex:)]) {
        return [self.delegate CX_photoBrowser:self placeholderImageForIndex:index];
    }
    return nil;
}

- (NSURL *)CX_highQualityImageURLForIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(CX_photoBrowser:highQualityImageURLForIndex:)]) {
        return [self.delegate CX_photoBrowser:self highQualityImageURLForIndex:index];
    }
    return nil;
}
