...
-(void)initSubView{
   ...
   //长按拖动排序手势
    ...
    [self.topViewBottomLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(KWidth, 0.5));
         ...
         usingSpringWithDamping:0.9 // 类似弹簧振动效果 0~1
          initialSpringVelocity:0.3 // 初始速度
                     ...
                     animations:^{
                         ...
                         self.frameView.frame=CGRectMake(0,20, KWidth, KHeight);
...
-(void)close{
    [UIView animateWithDuration:0.4 animations:^{
        self.frameView.frame=CGRectMake(0,KHeight, KWidth, KHeight);
        self.hidden=YES;
...
- (void) handlePan:(UIPanGestureRecognizer*) pan
{
    ...
    if (pan.state == UIGestureRecognizerStateBegan)
    {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.frameView.frame=CGRectMake(0, point.y, KWidth, KHeight);
        }];
...
-(UICollectionView *)collectionView{
    if(!_collectionView){
        ...
        flowLayout.headerReferenceSize = CGSizeMake(KWidth, 50);//UICollectionView header 的大小
        _collectionView=[[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, KWidth, KHeight-44-20) collectionViewLayout:flowLayout];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat itemWidth = (KWidth - 15 - 20-20) / 4;
